...
$$\boldsymbol{x}_{syn}=\boldsymbol{x}_p+\delta(\boldsymbol{x}_q-\boldsymbol{x}_p),\quad \boldsymbol{x}_p,\boldsymbol{x}_q\in S_{min},\quad \delta\sim U(0,1) $$
...
这里$\boldsymbol{x}_i,\boldsymbol{x}_j$为特征空间中的点，$\boldsymbol{K}$为核函数，为再生核希尔伯特空间上的内积。
...
$$
    \begin{align}
        &\min_{\alpha}\quad \frac{1}{2}\sum_{i,j=1}^N\alpha_i\alpha_jy_iy_j K(x_i,x_j)-\sum_{i=1}^N\alpha_i\\
        &s.t \quad \sum_{i=1}^N\alpha_iy_i=0\\
        &\quad \quad 0\le\alpha_i\le C,\quad i=1,2,\cdots,N
    \end{align}
$$
...
$$
    \begin{align}
        f(x)&=sign(\sum_{i=1}^{N_s}\alpha_i^{\ast}y_iK(x_i,x)+b^{\ast})
    \end{align}
$$
...
$$
    K=(K(\boldsymbol{x}_i,\boldsymbol{x}_j))_{(N+P)\times(N+P)}=
    \begin{bmatrix}
        K_1&K_2\\
        K_2^T&K_3
    \end{bmatrix}
...
这里，$K_1=(K(\boldsymbol{x}_i,\boldsymbol{x}_j))_{N\times N},\quad \boldsymbol{x}_i,\boldsymbol{x}_j\in S$

$K_2=(K(\boldsymbol{x}_i,\boldsymbol{x}_j))_{N\times P},\quad \boldsymbol{x}_i\in S,\boldsymbol{x}_j\in S^{syn}$

$K_3=(K(\boldsymbol{x}_i,\boldsymbol{x}_j))_{P\times P},\quad \boldsymbol{x}_i,\boldsymbol{x}_j\in S^{syn}$
...
#kernel_poly(data.matrix(yeast_tra)[1,3:ncol(yeast_tra)-1],data.matrix(yeast_tra)[2,3:ncol(yeast_tra)-1])
```
```{r}
...
#K_SMOTE(yeast_tra,yeast_test,2,3)
```
